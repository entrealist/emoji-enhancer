...
function translateAllTheThings () {
  ...
  if (document.getElementById('did-the-parsing-bit')) { return }
...
function getMeAnEmoji (keyword) {
  ...
  var maybeSingular = ''
  if (keyword[keyword.length - 1] === 's') { maybeSingular = keyword.slice(0, keyword.length - 1) }
  ...
  for (var emoji in allEmojis) {
    ...
    if (emoji === keyword || emoji === maybeSingular || emoji === maybePlural ||
        ...
        (keywords && keywords.indexOf(maybePlural) >= 0)) { return allEmojis[emoji].char }
  }
  return ''
};
